// Chart of Accounts (CoA) tree. Group nodes have type === "group" and no `code`.
// Account nodes carry `code`, `name`, `type` (Asset/Liability/Equity/Revenue/Expense),
// `fs` (BS/PL), `sec`, `bal` (Debit/Credit), and `active`.
export const COA = [
...
export function getActiveAccounts() {
  return COA.filter((node) => Boolean(node.code) && node.active !== false);
}
